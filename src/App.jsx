...
import { Upload, FileText, CheckCircle, XCircle, Download, ExternalLink, AlertCircle, LogOut, User, Trash2,  Settings, Home, DollarSign, ArrowRight } from 'lucide-react';
...
const [spendForm, setSpendForm] = useState({ cc:'', title:'', currency:'', approver:'', amount:'', category:'', atom:'', vendor:'', costCentre:'', region:'', timeSensitive:false, exceptional:'', justification:'', department:'' });
...
const [spendFilters, setSpendFilters] = useState({ status:'all', vendor:'all', category:'all', department:'all', dateFrom:'', dateTo:'', amountMin:'', amountMax:'', submittedBy:'all', approver:'all' });
const updateSpendFilter = (k,v) => setSpendFilters(p => ({...p,[k]:v}));
const clearSpendFilters = () => setSpendFilters({ status:'all', vendor:'all', category:'all', dateFrom:'', dateTo:'', amountMin:'', amountMax:'', submittedBy:'all', approver:'all' });
const getSpendFilterCount = () => { let c=0; if(spendFilters.status!=='all')c++; if(spendFilters.vendor!=='all')c++; if(spendFilters.category!=='all')c++; if(spendFilters.department!=='all')c++; if(spendFilters.dateFrom)c++; if(spendFilters.dateTo)c++; if(spendFilters.amountMin)c++; if(spendFilters.amountMax)c++; if(spendFilters.submittedBy!=='all')c++; if(spendFilters.approver!=='all')c++; return c; };
const [spendGroupBy, setSpendGroupBy] = useState('none');
const [showSpendColSelector, setShowSpendColSelector] = useState(false);
const [spendVisibleCols, setSpendVisibleCols] = useState({ title:true, vendor:true, amount:true, invoiced:true, category:true, department:true, submittedBy:true, date:true, status:true, approver:true, region:false, costCentre:false, atom:false });
...
setCurrentPage('landing'); setInvoices([]);
setSelectedFiles([]);
...
if (spendSearch) { const q=spendSearch.toLowerCase(); if(!s.title.toLowerCase().includes(q) && !s.vendor.toLowerCase().includes(q) && !s.submittedBy.toLowerCase().includes(q)) return false; }
...
{s.status === 'Approved' && canAssignInvoices() && (() => { const unlinkedInvs = invoices.filter(i => !i.spendApprovalId); return unlinkedInvs.length > 0 ? (<div className="mt-4 pt-3 border-t border-gray-200"><label className="text-xs font-semibold text-gray-500 uppercase mb-1 block">Assign Invoice</label><select defaultValue="" onChange={e => { if (e.target.value) { const invId = Number(e.target.value); acceptMatch(invId, s.id); const inv = invoices.find(i=>i.id===invId); setSelectedSpend({...s}); } }} className={`w-full ${_g}`}><option value="" disabled>Select an unlinked invoice...</option>{unlinkedInvs.map(i => (<option key={i.id} value={i.id}>{i.invoiceNumber} — {i.vendor} (${i.amount})</option>))}</select></div>) : null; })()}</div> {(s.status === 'Pending' || s.status === 'Escalated') && canApproveSpend() && (s.status !== 'Escalated' || user.isCeo || user.role === 'Admin') && (<div className="flex space-x-3 pt-4 border-t border-gray-200"> <button onClick={() => { updateSpendStatus(s.id,'Approved'); setSelectedSpend({...s,status:'Approved'}); }} className="px-5 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-semibold">{s.status === 'Escalated' ? 'Final Approve' : 'Approve'}</button>
...
<div><label className={_lb}>Approver</label><select value={spendFilters.approver} onChange={e => updateSpendFilter('approver',e.target.value)} className={`w-full ${_g}`}><option value="all">All</option>{[...new Set(spendApprovals.map(s=>s.approver))].sort().map(n=>(<option key={n} value={n}>{n}</option>))}</select></div></div> <div className="mt-4 pt-4 border-t border-gray-200"><p className="text-sm text-gray-600">Showing {filteredSpends.length} of {spendApprovals.length} requests</p></div></div>)}</div>
...
{(() => { const renderLookup = (title, items, setItems, editId, setEditId, newItem, setNewItem, prefix, hasCode=true, maxLen=5, placeholder='Code') => (
<div><div className={_fj+" mb-4"}><h2 className="text-xl font-bold text-gray-800">{title}</h2></div>
...
{invoice.status === 'Pending' && canApproveReject() && ( <div className={_cd}> <h2 className={_h2}>Actions</h2> <div className="space-y-3"> <button
onClick={() => updateApprovalStatus(invoice.id, 'Approved', user.name)} className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold" > <CheckCircle className="w-5 h-5"/> <span>Approve Invoice</span></button> <button
onClick={() => updateApprovalStatus(invoice.id, 'Rejected', user.name)} className="w-full flex items-center justify-center space-x-2 px-4 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 transition font-semibold" > <XCircle className="w-5 h-5"/> <span>Reject Invoice</span></button></div></div>)}
{invoice.status === 'Pending' && !canApproveReject() && ( <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4"> <p className="text-sm text-yellow-800"> <strong>View Only:</strong> Insufficient permissions.</p></div>)}
...
onClick={processInvoiceBatch} className="w-full bg-indigo-600 text-white py-3 rounded-lg font-semibold hover:bg-indigo-700"  > Process All Invoices</button></div>)}</div> <div className={_cd}> <div className={_fj+" mb-6"}> <h2 className="text-2xl font-bold text-gray-800">Invoice Tracking</h2> <div className="flex items-center space-x-3"> {selectedInvoiceIds.length > 0 && canApproveReject() && ( <div className={_fx}>
<span className="text-sm text-gray-600">{selectedInvoiceIds.length} selected</span> <button
onClick={() => bulkUpdateApprovalStatus('Approved', user.name)} className="flex items-center space-x-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700"  > <CheckCircle className="w-4 h-4"/> <span>Bulk Approve</span></button> <button
onClick={() => bulkUpdateApprovalStatus('Rejected', user.name)} className="flex items-center space-x-1 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700"  > <XCircle className="w-4 h-4"/> <span>Bulk Reject</span></button></div>)}
...
<div className="overflow-x-auto"> <table className="w-full"> <thead className="bg-gray-50"> <tr> {visibleColumns.checkbox && canApproveReject() && ( <th className="px-4 py-3 text-left"> <input type="checkbox"
checked={selectedInvoiceIds.length === groupInvoices.filter(inv => inv.status === 'Pending').length && groupInvoices.some(inv => inv.status === 'Pending')}
onChange={toggleSelectAll} className="w-4 h-4 text-indigo-600 rounded cursor-pointer"/></th>)}
...
{canApproveReject() && ( <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700">Actions</th>)}</tr></thead> <tbody className="divide-y divide-gray-200"> {groupInvoices.map((invoice) => ( <tr key={invoice.id} className="hover:bg-gray-50"> {visibleColumns.checkbox && canApproveReject() && ( <td className="px-4 py-3"> {invoice.status === 'Pending' && ( <input type="checkbox"
checked={selectedInvoiceIds.includes(invoice.id)}
onChange={() => toggleInvoiceSelection(invoice.id)} className="w-4 h-4 text-indigo-600 rounded cursor-pointer"/>)}</td>)}
...
{canApproveReject() && ( <td className="px-4 py-3 text-sm"> {invoice.status === 'Pending' && ( <div className="flex space-x-2"> {canApproveReject() && ( <> <button
onClick={() => updateApprovalStatus(invoice.id, 'Approved', user.name)} className="flex items-center space-x-1 px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700"  > <CheckCircle className="w-4 h-4"/> <span>Approve</span></button> <button
onClick={() => updateApprovalStatus(invoice.id, 'Rejected', user.name)} className="flex items-center space-x-1 px-3 py-1 bg-red-600 text-white rounded hover:bg-red-700"  > <XCircle className="w-4 h-4"/> <span>Reject</span></button> </>)}
{canDeleteInvoices() && ( <button
...
title="Delete Invoice" > <Trash2 className="w-4 h-4"/></button>)}</div>)}
{invoice.status !== 'Pending' && ( <span className="text-sm text-gray-500"> By {invoice.approvedBy}</span>)}</td>)}</tr> ))}</tbody></table></div></div> ))}</div>)}</div></div></div>);};
